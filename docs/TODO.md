...
- [x] `docs/PRD.md` 및 `docs/TODO.md` 현행화 (2026-08-27, 로컬 `pnpm dev`가 검색 워커 자동 기동)
- [x] `docs/PRD.md` 및 `docs/TODO.md` 현행화 (2026-08-27, 검색·검색 작업 메뉴 이원화)
- [x] `docs/PRD.md` 및 `docs/TODO.md` 현행화 (2026-08-27, 라이브 검색 이탈 시 중단·결과 보기 새 탭)
- [x] `docs/PRD.md` 및 `docs/TODO.md` 현행화 (2026-08-27, 입찰 열 권장가 제거·노출가 자동입력·BID 즉시 제출)
- [x] `docs/PRD.md` 및 `docs/TODO.md` 현행화 (2026-08-27, 입찰이 노출가 조회 호출 한도에 밀리지 않음)
- [x] `docs/PRD.md` 및 `docs/TODO.md` 현행화 (2026-08-27, 입찰 관리에 검색 보드 재고 보유 표기)
- [x] `docs/TODO.md` 조사 (2026-08-27, northface 백그라운드 검색 실패는 몰이 아니라 브랜드 ID 미매칭)
- [x] `docs/PRD.md` 및 `docs/TODO.md` 현행화 (2026-08-27, 브랜드 검색 명칭 매칭·0건은 실패)
- [x] `docs/TODO.md` 조사 (2026-08-28, JKJJX25012 노출가는 leakPrice가 아니라 통계 최저가 폴백)
- [x] `docs/PRD.md` 및 `docs/TODO.md` 현행화 (2026-08-27, 내장 워커 HMR 교체·옛 코드 0건 완료)
- [x] `docs/PRD.md` 및 `docs/TODO.md` 현행화 (2026-08-27, 검색 결과 품번 번호·페이지당 100)
- [x] `docs/PRD.md` 및 `docs/TODO.md` 현행화 (2026-08-27, 입찰 관리 재고 열 상시 표시)
- [x] `docs/PRD.md` 및 `docs/TODO.md` 현행화 (2026-08-27, 입찰 재고 열 맨 앞·SKU 별칭 매칭)
- [x] `docs/PRD.md` 및 `docs/TODO.md` 현행화 (2026-08-28, 아이템 발굴 테스트)
- [x] `docs/PRD.md` 및 `docs/TODO.md` 현행화 (2026-08-28, 대량 조회)
- [x] `docs/PRD.md` 및 `docs/TODO.md` 현행화 (2026-08-28, 대시보드 응답성)
- [x] `docs/PRD.md` 및 `docs/TODO.md` 현행화 (2026-08-28, 입찰 성공은 실데이터 확정 후에만)
...
- [x] Item Search `comingSoon` 제거 — 품번·브랜드 조회는 `/dashboard`. 수익 후보 발굴은 `/dashboard/discover` (2026-08-28)
...
- [x] **입찰 관리** (PRD §5.8) — 공식 목록·가격 수정·취소·CSV
  ...
  - [x] 검색 보드 `재고 보유` 표기(`sku_status.stock_marked`)를 입찰 **재고** 열에 항상 표시(보유 배지·날짜 / 미보유). 열은 체크박스 다음 맨 앞. `skuId`/`globalSkuId`/`dwSkuId`/`regionSkuId` 별칭으로 검색 보드 표기와 맞춘다. 검색·입찰 관리 모두에서 토글. 현재 페이지 필터 `재고 보유` (2026-08-27)
...
- [x] **중복 입찰 확인 팝업** — 로컬 이력 사전 확인 + 제출 후 실데이터 `91800500` 시 그대로 시도 / 가격 변경 재입찰 / 취소. BID 클릭에서 목록 전수 조회는 하지 않음 (2026-08-27)
...
- [x] 목표 마진율 입력 시 역산으로 권장 입찰가 도출 로직 추가 (2026-08-24)
  ...
  - `recommendBidFromCost`는 유지. **입찰 열 `권장 ₩`는 2026-08-27 제거** — 노출가 초과분은 판매되지 않음
- [x] 입찰 입력 기본값 = 중국 노출가, BID는 목록 전수 조회 없이 제출 (2026-08-27)
  - 자동입력: `leakPrice`(없으면 표시 중인 노출가 폴백). 수정 가능. 사용자 입력은 덮지 않음
  - BID 클릭이 `retrieve-bid` 목록을 최대 10페이지 훑던 대기가 버튼을 잠가 입찰이 안 되는 것처럼 보임. 로컬 이력으로만 사전 확인, 실데이터 중복은 제출 후 팝업
...
- [x] **로컬 워커 자동 기동** (2026-08-27) — `pnpm dev`만 켜도 큐가 진행되도록 `instrumentation.ts`가 development Node에서 워커 루프를 시작.
      공유 코어는 `lib/search/worker-loop.ts`. `DISABLE_DEV_WORKER=1`이면 내장 루프를 끄고 `pnpm worker`만 사용.
      이미 Next만 떠 있는 세션은 재시작 전까지 대기 잡이 멈춘다.
...
- [x] **조사** `northface` 백그라운드 실패 (2026-08-27) — 무신사·29CM·W컨셉만 켠 것과 무관.
      잡 `69c688d8`은 POIZON `brand/page/by-name`이 `northface`(붙여 씀)에 대해 브랜드 ID를 못 찾음.
      원가 몰 수집 전에 실패(적재 0). `kolon`/`Nike`는 같은 API로 ID가 나옴. 경고 삼각형에 동일 문구.
- [x] **브랜드 매칭·0건 완료** (2026-08-27) — `The north face`가 `Face a Face`(1044891, 상품 0)로 잡혀 `done` 0건.
      원인: 후보 첫 `isShow=1`을 그대로 씀. 실제 노스페이스는 ID 45.
      명칭 유사도(+별칭)로 고르고, 첫 페이지 0건은 실패. 브랜드/품번 없음 등 결정적 오류는 잡 재시도 안 함
- [x] **내장 워커가 옛 코드를 붙잡음** (2026-08-27) — `startDevSearchWorker`가 한 번만 기동해 매칭 수정 후에도 0건 `done`.
      HMR 시 루프 세대(generation)로 교체. 이미 떠 있는 프로세스는 개발 서버 재시작 필요
- [x] 완료 알림 — 사이드바 `검색 작업` 배지 (진행 중 파란색 / 미확인 완료 초록색)
- [x] 결과 인계 (구) — `결과 보기` → `/dashboard?job=<id>` → 검색 화면이 잡 결과를 그대로 로드.
      **2026-08-27 이원화로 폐기.** 잡 결과는 `/dashboard/jobs/<id>` (검색 작업 메뉴). 검색 메뉴 `items`는 덮지 않음. §10.7
...
- 목록 필드가 같으면 `setJobs`를 생략한다. 검색 보드는 `refresh`만 구독하고 목록 변경으로 결과 테이블을 다시 그리지 않는다
...
- [x] 배포 시 Route Handler + 크론으로 워커 이식 — `app/api/cron/search-worker/route.ts`
      ...
      (로컬은 `pnpm dev` 내장 워커 또는 `pnpm worker`. 서버리스는 호출당 1잡·maxDuration 내 완료)
...
- [x] 목표 마진율 역산 권장 입찰가 (2026-08-24, §6) — **입찰 열 표기는 2026-08-27 철회**
  - 마진 설정의 목표 마진율·역산 로직은 유지. 입찰 열에는 노출가 자동입력 + 실수령만

### 10.7 [검색·검색 작업 메뉴 이원화] (2026-08-27)

> 원인: 조회 중 `결과 보기`가 `/dashboard?job=`로 검색 보드 `items`를 교체하고, 페이지 이탈 시 조회 세션이 언마운트됨.
> PRD §5.0·§5.6: 두 메뉴는 독립 상태. 전환은 탭(메뉴)이며 분할/드로어/모달이 아님.

- [x] **D1** 대시보드 keep-alive — `/dashboard`·`/dashboard/jobs` 보드를 레이아웃에 숨긴 채로 유지. 메뉴 전환·다른 대시보드 페이지 이동 후에도 현재 조회·선택한 잡 결과가 남음
- [x] **D2** 조회 `중단` — 검색 툴바에 `중단`. 진행 중 `조회`/`더 불러오기`만 무시하고 이미 모인 목록은 유지. 메뉴 전환·잡 결과·잡 링크는 조회를 취소하지 않음
- [x] **D3** 잡 결과는 검색 작업 메뉴 — `결과 보기`·푸시·구 `/dashboard?job=` → `/dashboard/jobs/<id>`. 검색 보드 `usePoizonSearch`는 잡 payload를 읽지 않음
- [x] **D4** 잡 결과 보드 — `/dashboard/jobs/<id>`에서 입찰·스킵·검토·선택 가격 갱신·손댄 품번 합치기. `목록으로`로 잡 목록 복귀. 라이브 `items`와 상태 분리
- [x] **D5** 브라우저 검증 — 검색 작업 결과 보드가 검색 메뉴를 덮지 않음. `중단` 버튼 표시. 잡 보드에 일괄 입찰. 메뉴 전환 keep-alive는 클라이언트 내비게이션으로 확인

### 10.8 [라이브 검색 이탈 시 중단 · 결과 보기 새 탭] (2026-08-27)

> 원인: keep-alive가 검색 보드를 숨긴 채로 유지해, 다른 메뉴에서도 조회·추천가·원가 오퍼가 이어져 화면이 느려짐.
> PRD §5.0: 라이브 조회는 검색 화면을 나가면 중단. 백그라운드 잡만 워커가 계속. `결과 보기`는 새 탭.

- [x] **E1** keep-alive 제거 — `/dashboard`·`/dashboard/jobs`·`/dashboard/jobs/<id>`가 각자 페이지를 렌더. 검색 메뉴를 나가면 라이브 보드 언마운트
- [x] **E2** 라이브 조회 중단 — 언마운트 시 `조회` runId 무효화, 추천가 큐 비우기, 원가 오퍼 응답 무시. 워커 잡은 그대로 진행
- [x] **E3** `결과 보기` 새 탭 — `/dashboard/jobs/<id>`를 `target="_blank"`로 연다. 목록 탭은 유지되고 여러 잡 결과를 탭 전환으로 본다
- [x] **E4** 브라우저 검증 — 검색 화면 이탈 후 라이브 조회가 이어지지 않음. 결과 보기 N회 → 브라우저 탭 N개. 백그라운드 잡은 검색 화면과 무관하게 진행

### 10.9 [입찰 열 권장가 제거 · BID 즉시 제출] (2026-08-27)

> 원인: BID가 `getExistingBidsForSkus`로 실데이터 활성 입찰 목록을 최대 10페이지 훑는 동안 `isBidding`이 켜져 버튼이 잠김. 추천가 API 혼잡 시 수 분 대기. 빈 입력의 `권장 ₩`가 행 높이(`h-12`)를 넘어 다음 행 BID를 가로챔.
> PRD §5.1·§5.3·§5.5: 입찰 기본값=노출가. 권장가는 입찰 열에 없음. 목록 전수 조회를 기다리지 않음.

- [x] **F1** 입찰 열 `권장 ₩` 제거. 입력은 중국 노출가로 자동 채움(수정 가능)
- [x] **F2** BID/일괄 입찰은 로컬 이력만 사전 확인. 실데이터 중복은 제출 후 팝업
- [x] **F3** 브라우저 검증 — 권장 문구 없음, 입력이 노출가, BID가 잠기지 않음

### 10.10 [입찰이 노출가 조회 호출 한도에 밀림] (2026-08-27)

> 원인: 옵션 뷰·워커가 `recommend-bid/price`를 대량 호출하고, `400010007`을 3×3회 재시도해 한도를 채움. 입찰 이력 표시도 실데이터 목록을 훑어 같은 한도를 씀. 로그에 `submit-bid`가 없는 것은 BID가 제출 전에 막히기 때문.
> PRD §5.3·§5.6: 잡 결과 보드는 payload 노출가. 빈도 제한은 재시도하지 않음. BID는 노출가 큐와 무관.

- [x] **G1** 검색 작업 결과 보드에서 노출가 재조회 안 함 (`선택 가격 갱신`만)
- [x] **G2** `400010007`/`频次超限`은 재시도하지 않음. 추천가 `withRetry` 중첩 제거
- [x] **G3** 입찰 이력 화면 표시는 로컬 `bid_history`만. 목록 전수 조회 제거
- [x] **G4** BID 중 추천가 큐 일시 정지. 워커 노출가 동시성 5→2

### 10.11 [검색 결과 품번 번호 · 페이지당 100] (2026-08-27)

> 백그라운드 수집이 품번 500개까지 모이면 한 화면에 전부 그리기 어렵다. 번호는 옵션이 아니라 품번 기준.

- [x] **H1** 검색 보드 첫 열에 품번 순번(`#`). 필터·정렬된 품번 1부터. 옵션 뷰는 같은 품번 동일 번호, 펼친 옵션은 번호 없음
- [x] **H2** 품번 기준 한 페이지 최대 100개. 푸터 페이지 이동. 전체 선택·이력·노출가 큐는 현재 페이지

### 10.12 [아이템 발굴 테스트] (2026-08-28)

> 검색은 아는 품번·브랜드 조회 작업대. 발굴은 유통·인기·순수익 하한을 통과한 후보만 모아 빨리 입찰. 실데이터에 전체 카탈로그·랭킹 API 없음 → 씨드는 브랜드 1개.

- [x] **I1** `search_jobs.options`: `purpose` / `minNetProfit` / `minSalesVolume`. 목록은 purpose로 분리
- [x] **I2** 워커: 발굴만 가격 수집 후 적재 필터. 검색 잡은 미변
- [x] **I3** `/dashboard/discover` 메뉴·등록·목록, `/dashboard/discover/<id>` 결과 보드(즉시 입찰)
- [x] **I4** 브라우저 검증 — 메뉴 분리, 백그라운드 수집, 검색 작업 목록 미혼입, 결과 보드 `발굴 결과`·수익 옵션·입찰. 첫 페이지 적재는 실데이터 `400010007` 한도로 대기 중. 내장 워커는 재시작해야 새 적재 필터를 탐
- [x] **조사** `JKJJX25012`(kolon 발굴 잡 `bf444973-…`): 화면 노출가 ≠ 실데이터 「중국 구매자 페이지 노출 보장」
  - 의도 매칭: SKU = `recommend-bid/price` `leakInfos[CN].leakPrice`. 접힌 품번 = 통계 `minPrice/marketPrice`
  - 실제 표시: 이 잡 `skuRecommendations` 0/287품번·0/3132 SKU. 워커 `400010007`로 노출가 조회 실패(경고 67건). 잡 보드는 재조회 안 함(G1)
  - 폴백: `skuListPrice` = 통계 `minPrice.globalMinPriceVO` (최저 입찰가). 라이트옐로우 100/105 ₩243,000 vs 실데이터 노출 보장 ₩239,000. 90/95는 두 값이 같아 ₩234,000으로 일치처럼 보임. 접힌 품번 ₩244,000은 통계 최저가
  - 라이브 `recommend-bid/price` 재확인은 같은 한도로 실패. 한도 해제 후 `선택 가격 갱신` 또는 `pnpm dump:recommend`로 leak 대조 필요
- [ ] **이월** 씨드 확장(`by-categoryId`·다중 브랜드), 추출 조건 살붙이기. 발굴 잡 노출가 0건 폴백(통계 최저가를 노출 보장처럼 보여 줌) 완화

### 10.13 [대량 조회] (2026-08-28)

> 실데이터 판매 엑셀(행=옵션, 열 `상품 번호`)을 올리면 품번만 모아 검색과 동일한 보드에서 최저가·원가·입찰·검토. 라이브 쉼표 입력은 대량에 쓰지 않고 백그라운드 품번 잡(`purpose=bulk`)으로 돌린다.

- [x] **J1** `.xlsx`에서 `상품 번호` 유니크 추출(끝 한자는 0건일 때 재시도, 상한 500). 각 품번 전체 옵션
- [x] **J2** `/dashboard/bulk` 메뉴·업로드·목록, `/dashboard/bulk/<id>` 결과 보드. 검색 작업·발굴과 목록 분리
- [x] **J3** 브라우저 검증 — 샘플 엑셀 309행→14품번, 메뉴 분리, 결과 보드 9건·42옵션·입찰·검토. 호출 한도로 4품번 부분 완료
- [x] **J4** 품번 잡 20건씩 청크 적재. 진행률은 조회한 품번 수. 워커 중단 후 `articleOffset`부터 이어 감. 재실행 시 offset 0

### 10.14 [대시보드 응답성] (2026-08-28)

> 원인: 잡 폴링이 검색 보드 전체를 구독하고, 입찰가 한 글자·추천가 1건마다 페이지 행을 다시 그렸으며, 메뉴 전환에 `loading.tsx`가 없어 이전 화면이 멈춘 것처럼 보였다. 클릭 액션은 `users.id`를 매번 조회했다.
> 제약: 입찰·스킵·검토·잡 진행률·워커 자동 기동 등 기능·결과는 동일.

- [x] **K1** 잡 목록 API 콘텍스트 분리. 동일 목록은 setState 생략. 검색 보드는 `refresh`만
- [x] **K2** 입찰가 입력은 행 로컬, 추천가/원가 응답은 프레임 단위 배치. 테이블 context 안정화
- [x] **K3** `app/dashboard/loading.tsx`. `getCurrentUserId` TTL 캐시 (클릭 액션 공통)
- [ ] **이월** 행 가상 스크롤. 개발 워커를 Next 이벤트 루프와 프로세스 분리 (기동은 `pnpm dev` 유지)

### 10.15 [입찰 성공 메시지 vs 실입찰 없음] (2026-08-28)

> 발굴 보드에서 「입찰 성공」이 떴는데 실데이터 입찰 관리에는 건이 없음.
> 원인: `submit-bid` `code === 200`만 보고 성공 처리. `sellerBiddingNo` 없이 `bid_history`·낙관적 배지(`입찰 n/m`)를 올림. 공식 문서는 입찰번호로 최종 상장 여부를 확인하라고 함.

- [x] **L1** 성공은 입찰번호 + 실데이터 목록(활성·거래중) 확인 뒤에만. 번호 없는 200은 실패
- [x] **L2** 제출 페이로드: DW `skuId` 우선, 보드 키가 다르면 `globalSkuId` 동반. 로컬 이력 키는 화면 SKU 유지
- [ ] **이월** 실데이터 입찰 1건으로 발굴 보드 BID → 입찰 관리 활성 탭 대조 (라이브 상장은 여기서 하지 않음)
