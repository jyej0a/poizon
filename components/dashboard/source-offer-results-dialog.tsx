...
interface SourceOfferResultsDialogProps {
  isOpen: boolean;
  onClose: () => void;
  items: SourceOffer[] | null;
  articleNumber?: string;
}

export function SourceOfferResultsDialog({
  isOpen,
  onClose,
  items,
  articleNumber,
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-2xl max-h-[85vh] flex flex-col gap-0 p-0 overflow-hidden border-secondary/40 shadow-2xl">
        <DialogHeader className="px-5 py-4 border-b bg-emerald-500/[0.04] text-left">
          ...
          {articleNumber && (
            <DialogDescription className="font-mono text-xs">품번 {articleNumber}</DialogDescription>
          )}
        </DialogHeader>
        ...
        <DialogFooter className="px-5 py-3 border-t bg-secondary/5 sm:justify-end gap-2">
          <button
            type="button"
            onClick={onClose}
